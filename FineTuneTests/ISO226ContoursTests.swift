...
struct LoudnessCompensatorHybridTests {
    ...
    func referenceVolumeIsLinear() {
        ...
        processor.updateForVolume(1.0, mode: .modern)
    ...
    @Test("At low volume in Modern Mode, low exciter generates non-linear 3rd harmonic")
    func lowVolumeGeneratesLowHarmonicsInModernMode() {
        let sampleRate = 48000.0
        let processor = LoudnessCompensator(sampleRate: sampleRate)
        
        // System volume = 0.0, Modern mode (maximum exciter intensity)
        processor.updateForVolume(0.0, mode: .modern)
        
        // Generate a 50 Hz sine wave (falls within the Low-Pass crossover filter at 100 Hz)
        let frameCount = 1024
        var input = [Float](repeating: 0.0, count: frameCount * 2)
        var output = [Float](repeating: 0.0, count: frameCount * 2)
        ...
        let magnitude3rdHarmonic = sqrt(realSum * realSum + imagSum * imagSum) / Double(frameCount)
        // Verify that the 3rd harmonic is clearly present (> 0.001)
        #expect(magnitude3rdHarmonic > 0.001, "In Modern mode at 0% volume, 3rd harmonic should be generated by Low Exciter, got \(magnitude3rdHarmonic)")
    }

    @Test("At low volume in Classic Mode, processing is purely linear (exciter is off)")
    func lowVolumeIsLinearInClassicMode() {
        let sampleRate = 48000.0
        let processor = LoudnessCompensator(sampleRate: sampleRate)
        
        // System volume = 0.0, Classic mode (100% linear EQ, no exciter)
        processor.updateForVolume(0.0, mode: .classic)
        ...
        let magnitude3rdHarmonic = sqrt(realSum * realSum + imagSum * imagSum) / Double(frameCount)
        // Verify that in Classic Mode no harmonics are generated (magnitude < 1e-4)
        #expect(magnitude3rdHarmonic < 1e-4, "In Classic mode at 0% volume, 3rd harmonic should be negligible, got \(magnitude3rdHarmonic)")
    }
}

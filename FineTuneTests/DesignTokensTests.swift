...
struct DesignTokensDimensionTests {
    ...
    func outputLevelMeterRedThresholds() {
        ...
        #expect(OutputLevelMeter.labelDBs == [-30, -20, -15, -10, -6, -3, 0])
        #expect(abs(meter.db(forSegment: meter.firstRedSegmentIndex)) < 0.001)
        #expect(abs(meter.xPosition(for: 0) - 319.0) < 0.001)
        #expect(abs(meter.xPosition(for: -30) - 1.0) < 0.001)
        #expect(abs(meter.xPosition(for: -20) - 43.0) < 0.001)
        #expect(abs(meter.xPosition(for: -15) - 91.0) < 0.001)
